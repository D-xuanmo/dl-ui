...
export default defineComponent({
  ...
  setup(props, context: SetupContext<EventType>) {
    ...
    // 接收子级传递回来的数据，用作缓存
    const temporaryValue = ref<PickerValueType>(innerValue.value && innerValue.value.length > 0 ? deepCopy(innerValue.value) : findCascadeFirstLevelData(props.columns as CascadeDataType[]))
    // 内部渲染列使用
